"""Environment/config diagnostics for HindiBabyNet-Pipeline."""
...
def main() -> None:
    ...
    try:
        ...
        lines.append(_status("Prepared audio root", True, str(processed_root)))
    except Exception as exc:
        lines.append(_status("Prepared audio root", False, str(exc)))
        has_error = True

    try:
        raw_joined_root = cfg.get_raw_joined_audio_root()
        raw_joined_root.mkdir(parents=True, exist_ok=True)
        lines.append(_status("Raw joined audio root", True, str(raw_joined_root)))
    except Exception as exc:
        lines.append(_status("Raw joined audio root", False, str(exc)))
        has_error = True
    ...
    for label, path_getter in [
        ("TextGrid output root", cfg.get_textgrid_output_root),
        ("Manual annotation root", cfg.get_manual_annotation_root),
        ("Evaluation output root", cfg.get_evaluation_output_root),
    ]:
        try:
            root = path_getter()
            root.mkdir(parents=True, exist_ok=True)
            lines.append(_status(label, True, str(root)))
        except Exception as exc:
            lines.append(_status(label, False, str(exc)))
            has_error = True
