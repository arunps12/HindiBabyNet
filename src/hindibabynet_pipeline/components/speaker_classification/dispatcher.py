"""
Backend dispatcher â€” select the right Stage 03 backend from config.
...
def get_backend(
    ...
    if name == "xgb":
        from hindibabynet_pipeline.components.speaker_classification.xgb_backend import XGBBackend
        return XGBBackend(cfg)

    if name == "vtc":
        from hindibabynet_pipeline.components.speaker_classification.vtc_backend import VTCBackend
