...
Full pipeline per analysis WAV:
  Step 2  â€“ WebRTC VAD  â†’  speech intervals
  Step 3  â€“ Chunked pyannote diarization  â†’  speaker turns
  Step 4  â€“ Intersect VAD âˆ© diarization, drop < min_segment_sec, merge close
  Step 5  â€“ eGeMAPS + XGBoost classification  â†’  adult_male / adult_female / child / background
  Step 6  â€“ Aggregate class streams
  Step 7  â€“ Secondary diarization on female/male streams  â†’  main speaker
  Step 8  â€“ Export main_female.wav, main_male.wav
  Step 9  â€“ TextGrid generation
...
from hindibabynet_pipeline.entity.artifact_entity import SpeakerClassificationArtifact
from hindibabynet_pipeline.entity.config_entity import SpeakerClassificationConfig
...
from hindibabynet_pipeline.utils.textgrid_utils import intervals_to_df, write_textgrid
...
# ============================================================================
# Step 2 â€” VAD  (exact notebook: webrtc_vad_regions_streaming)
# ============================================================================
# webrtc_vad_regions is now in hindibabynet_pipeline.utils.audio_utils


# ============================================================================
# Step 3 â€” Chunked diarization  (exact notebook logic)
# ============================================================================
def _make_chunks(
...
# ============================================================================
# Step 4 â€” Intersect + merge  (exact notebook logic)
# ============================================================================
...
# ============================================================================
# Step 5 â€” eGeMAPS feature extraction + classification  (exact notebook)
# ============================================================================
@dataclass
...
def generate_windows(
    ...
    Notebook windowing strategy:
      - Short segments (< win) â†’ single padded window.
      - Long segments â†’ 1.0 s windows with 0.5 s hop + end-anchored last window.
...
# ============================================================================
# Step 6 â€” Build combined class-stream audio  (notebook: build_combined_audio)
# ============================================================================
def build_class_stream(
    ...
    Returns
    -------
    audio : np.ndarray          â€“ concatenated stream
    used_df : pd.DataFrame      â€“ rows that were included
...
# ============================================================================
# Step 7 â€” Secondary diarization â†’ find main speaker
# ============================================================================
def extract_main_speaker_audio(
    ...
    # â”€â”€ Write dominant-speaker audio â”€â”€
    ...
    # â”€â”€ Map dominant speaker segments back to original recording time â”€â”€
    ...
    for stream_s, stream_e, orig_s, orig_e in offset_table:
        ...
        for ms, me in main_segs:
            ...
            if o_e > o_s:
                overlap += o_e - o_s
        # Keep if dominant speaker covers â‰¥ 50% of the segment
...
class SpeakerClassification:
    ...
    Steps: VAD â†’ diarization â†’ intersect â†’ merge â†’ classify â†’ aggregate â†’
           secondary diar â†’ export wavs â†’ TextGrid
    """
    ...
    def _load_model(self):
        ...
        # Force model to CPU â€” input is always numpy (CPU) arrays.
    ...
    def run(
        ...
        try:
            ...
            logger.info(f"[{participant_id}] Step 4: Intersect VAD âˆ© diarization, merge")
            ...
            # Female stream â†’ main_female.wav (dominant speaker only)
            ...
            # Male stream â†’ main_male.wav (dominant speaker only)
            ...
            # Child stream â†’ child.wav (all child segments, no secondary diar)
            ...
            # Background stream â†’ background.wav (all background segments, no secondary diar)
