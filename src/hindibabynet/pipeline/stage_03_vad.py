...
from src.hindibabynet.components.vad import VAD
...
def _is_complete(cfg: ConfigurationManager, run_id: str, pid: str) -> bool:
    vad_cfg = cfg.get_vad_config(run_id=run_id, participant_id=pid)
    return vad_cfg.vad_parquet_path.exists()


def _run_single(cfg: ConfigurationManager, run_id: str, wav_path: Path, pid: str):
    vad_cfg = cfg.get_vad_config(run_id=run_id, participant_id=pid)
    component = VAD(vad_cfg)
    return component.run(analysis_wav_path=wav_path, participant_id=pid)


def main():
    ap = argparse.ArgumentParser(description="Stage 03: Voice Activity Detection")
    ...
    add_file_handler(logger, cfg.get_logs_root() / run_id / "stage_03_vad.log")
    ...
    if args.wav:
        ...
        logger.info(f"Stage 03 (single) | wav={wav_path} pid={pid} run_id={run_id}")
    ...
    if args.analysis_dir:
        ...
        logger.info(f"Stage 03 batch (analysis_dir) | n={len(wav_files)} run_id={run_id}")
    ...
    if args.recordings_parquet:
        ...
        logger.info(
            f"Stage 03 batch (parquet) | n={len(participants)} "
