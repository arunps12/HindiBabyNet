...
def run_classify_xgb(
    *,
    ...
    """Run XGB speaker classification directly via the backend dispatcher."""
    ...
    add_file_handler(
        logger,
        cfg.get_logs_root() / run_id / f"xgb_classification_{run_id}.log",
    )
    ...
    logger.info(f"XGB classification finished | ok={ok} skip={skip} fail={fail}")